...
export default async function handler(req, res) {
  ...
  // Giorni di chiusura/ferie nell'intervallo
  const closures = await prisma.closure.findMany({
    where: { day: { gte: start, lte: end } },
    include: { group: { select: { name: true } } },
    orderBy: { day: "asc" },
  });
  const closureList = closures.map((c) => ({
    id: c.id,
    day: c.day,
    groupName: c.group ? c.group.name : null, // null = tutti i gruppi
    reason: c.reason,
  }));

  return res.status(200).json({ slots: data, groups: groupList, closures: closureList });
}
